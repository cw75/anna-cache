...
unsigned causal_comparison(
    const std::shared_ptr<MultiKeyCausalLattice<SetLattice<string>>> &lhs,
    const std::shared_ptr<MultiKeyCausalLattice<SetLattice<string>>> &rhs) {
...
std::shared_ptr<MultiKeyCausalLattice<SetLattice<string>>> causal_merge(
    const std::shared_ptr<MultiKeyCausalLattice<SetLattice<string>>> &lhs,
    const std::shared_ptr<MultiKeyCausalLattice<SetLattice<string>>> &rhs) {
  ...
  if (comp_result == kCausalGreaterOrEqual) {
    ...
    auto result = std::make_shared<MultiKeyCausalLattice<SetLattice<string>>>();
    result->merge(*lhs);
    result->merge(*rhs);
    return result;
  }
...
std::shared_ptr<MultiKeyCausalLattice<SetLattice<string>>>
...
bool populate_in_preparation(
    ...
    const std::shared_ptr<MultiKeyCausalLattice<SetLattice<string>>> &lattice,
...
void recursive_dependencies_check(
    const Key &head_key,
    const std::shared_ptr<MultiKeyCausalLattice<SetLattice<string>>> &lattice,
  ...
  for (const auto &pair : lattice->reveal().dependencies.reveal()) {
    Key dep_key = pair.first;
    // first, check if the dependencies is already satisfied in the causal cut
    if (causal_cut_store.find(dep_key) != causal_cut_store.end() &&
        vector_clock_comparison(
            ...
            lattice->reveal().dependencies.reveal().at(dep_key)) ==
            kCausalGreaterOrEqual) {
      continue;
    }
    // then, check if the dependencies is already satisfied in the
    // in_preparation
    ...
    if (target_lattice != nullptr) {
      if (populate_in_preparation(head_key, dep_key, target_lattice,
                                  in_preparation)) {
        recursive_dependencies_check(head_key, target_lattice, in_preparation,
                                     causal_cut_store, unmerged_store,
                                     to_fetch_map, cover_map, client, log);
      }
      // in_preparation[head_key].second[dep_key] = target_lattice;
    } else {
      // check if the dependencies is satisfied in unmerged_store (unmerged
      // store should always dominate the in_preparation)
      if (unmerged_store.find(dep_key) != unmerged_store.end() &&
        ...
        if (populate_in_preparation(head_key, dep_key,
                                    ...
                                    in_preparation)) {
          recursive_dependencies_check(head_key, unmerged_store.at(dep_key),
        ...
        cover_map[dep_key][lattice->reveal().dependencies.reveal().at(dep_key)]
            .insert(head_key);
...
Address
find_address(const Key &key, const VectorClock &vc,
...
void save_versions(const string &id, const Key &key,
  ...
  if (observed_keys.find(key) == observed_keys.end()) {
    ...
    for (const auto &pair :
         causal_cut_store.at(key)->reveal().dependencies.reveal()) {
...
bool fire_remote_read_requests(PendingClientMetadata &metadata,
  ...
  map<Address, KeyVersionRequest> addr_request_map;
  ...
  for (const Key &key : metadata.read_set_) {
    ...
    if (causal_cut_store.find(key) == causal_cut_store.end()) {
      ...
      CausalCacheThread cct(remote_addr, 0);
      remote_addr = cct.causal_cache_key_version_response_connect_address();
      ...
      if (addr_request_map.find(remote_addr) == addr_request_map.end()) {
        ...
        addr_request_map[remote_addr].set_response_address(
            cct.causal_cache_key_version_response_connect_address());
      }
...
void respond_to_client(
  ...
  if (version_store.find(pending_multi_key_metadata[addr].client_id_) !=
      version_store.end()) {
    for (const auto &pair :
         version_store.at(pending_multi_key_metadata[addr].client_id_)) {
...
void merge_into_causal_cut(
  ...
  for (const auto &addr : in_preparation[key].first) {
    if (pending_multi_key_metadata.find(addr) !=
      ...
      if (key_dne) {
        pending_multi_key_metadata[addr].dne_set_.insert(key);
      }
      pending_multi_key_metadata[addr].to_cover_set_.erase(key);
      if (pending_multi_key_metadata[addr].to_cover_set_.size() == 0) {
        // all keys are covered, safe to read
        // decide local and remote read set
        if (!fire_remote_read_requests(pending_multi_key_metadata[addr],
                                       version_store, causal_cut_store, pushers,
                                       cct, log)) {
          // all local
          respond_to_client(pending_multi_key_metadata, addr, causal_cut_store,
                            version_store, pushers, cct, unmerged_store);
        } else {
          client_id_to_address_map[pending_multi_key_metadata[addr].client_id_]
              .insert(addr);
        }
      }
    }
...
void process_response(
  ...
  if (unmerged_store.find(key) == unmerged_store.end()) {
    ...
    // check call back addresses for single_key obj causal consistency
    if (single_key_callback_map.find(key) != single_key_callback_map.end()) {
      // notify clients
      for (const auto &addr : single_key_callback_map[key]) {
        // pending_single_key_metadata[addr].to_cover_set should have this
        // key, and we remove it
        pending_single_key_metadata[addr].to_cover_set_.erase(key);
        if (pending_single_key_metadata[addr].to_cover_set_.size() == 0) {
          CausalResponse response;

          for (const Key &key : pending_single_key_metadata[addr].read_set_) {
            CausalTuple *tp = response.add_tuples();
            tp->set_key(key);
            if (vector_clock_comparison(
                    VectorClock(),
          ...
          kZmqUtil->send_string(resp_string, &pushers[addr]);
          // GC
          pending_single_key_metadata.erase(addr);
        }
      }
      single_key_callback_map.erase(key);
    }
  } else {
  ...
  if (to_fetch_map.find(key) != to_fetch_map.end() &&
    ...
    recursive_dependencies_check(key, unmerged_store[key], in_preparation,
                                 causal_cut_store, unmerged_store, to_fetch_map,
                                 cover_map, client, log);
    if (to_fetch_map[key].size() == 0) {
      // this key has no dependencies
      merge_into_causal_cut(key, causal_cut_store, in_preparation,
                            version_store, pending_multi_key_metadata, pushers,
                            cct, client_id_to_address_map, log, unmerged_store);
      to_fetch_map.erase(key);
    }
  }
  // then, check cover_map to see if this key covers any dependencies
  if (cover_map.find(key) != cover_map.end()) {
    std::unordered_map<VectorClock, set<Key>, VectorClockHash> to_remove;
    // track the head keys whose dependencies is NOT satisfied
    set<Key> dependencies_not_satisfied;
    // track the head keys whose dependencies might be satisfied
    set<Key> dependencies_may_be_satisfied;
    ...
    for (const auto &pair : cover_map[key]) {
      if (vector_clock_comparison(unmerged_store.at(key)->reveal().vector_clock,
        ...
        for (const auto &head_key : pair.second) {
          // found a dependencies that is covered
          in_preparation[head_key].second[key] = unmerged_store[key];
          if (to_fetch_map[head_key].find(key) ==
              to_fetch_map[head_key].end()) {
            log->error("Missing dependencies {} in the to_fetch_map of key {}.",
                       key, head_key);
          }
          dependencies_may_be_satisfied.insert(head_key);
          to_remove[pair.first].insert(head_key);
        }
      } else {
        for (const auto &head_key : pair.second) {
          dependencies_not_satisfied.insert(head_key);
        }
      }
    }
    // only remove from to_fetch_map if the dependencies is truly satisfied
    for (const Key &head_key : dependencies_may_be_satisfied) {
      if (dependencies_not_satisfied.find(head_key) ==
          dependencies_not_satisfied.end()) {
